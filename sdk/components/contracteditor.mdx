...
| `onDocumentSave`    | `function(data: {documentSaved:boolean})` |   |Called when the contract body is being edited, this function returns `documentSaved: false` if the value is not saved and `documentSaved: true` if the value is saved.|
